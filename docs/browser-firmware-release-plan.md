...
- QA 端末で `npm run check:browser-firmware:ui` を実行し、Build & Flash の button layout、右ペイン整理、artifact 由来表示、token 非保存/消去、layer 追加・複製・参照中 layer 削除ブロック・最後の layer 削除だけが使えることをレンダリング結果で確認する。

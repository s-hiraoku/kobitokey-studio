...
11. 公開判定に使う外部 E2E 証跡ファイルは、可能なら `BROWSER_FIRMWARE_E2E_OAUTH_CLIENT_ID=<GitHub OAuth client id> npm run collect:browser-firmware:e2e-report -- --out <report.json>` で生成する。QA 端末で production URL の rendered UI smoke も同時に実行できる場合は `--run-ui-smoke` を付ける。collector は production URL、KobitoKey Studio の Actions run URL、`production.appCommitSha`、API route の security header / Worker route、OAuth scope 制限、本番 Worker route 経由の OAuth device-code 発行、GitHub commit の変更ファイル一覧、Actions run の head SHA / head branch、Actions artifact の name / id / size / expiry、Actions artifact zip 内の UF2 entry name / artifact id / artifact name / SHA-256、manifest entry name / artifact id / artifact name / SHA-256、左右 UF2 の SHA-256 を収集し、生成後に validator へ通す。GitHub commit の変更ファイル一覧は、key だけを変えた場合は `config/KobitoKey.keymap` だけになるような実差分の一覧でよく、1 件以上かつ managed firmware files 以外を含まないことを validator で確認する。手動で作る場合は `docs/browser-firmware-e2e-evidence.template.json` を元に verifier、`?mode=firmware` 付き production URL、KobitoKey Studio の Actions run URL、production app commit SHA、CI run、GitHub run、artifact、flash method (`direct-copy` または `download-copy`)、flash 結果、Build & Flash / token 非保存と消去 / key editing / Combo / Trackball / release wizard precondition / artifact 由来表示 / 表示 artifact 名・id の build artifact 一致確認 / layer 操作 smoke 結果を埋める。token と UF2 bytes は記録しない。validator は template placeholder、Firmware Mode 以外の production URL、feature preview など expected public production origin 以外の URL、`production.fetchUrl` が `production.url` と違う証跡、KobitoKey Studio 以外の CI run URL、`production.appCommitSha` が `ci.appCommitSha` と違う証跡、production/API security headers 未確認、Worker OAuth device-code 発行未確認、GitHub commit / run URL の repository 不一致、Actions run の head SHA 不一致、Actions run の head branch と選択 branch の不一致、managed firmware files 以外を含む commit、missing / expired / empty Actions artifact、Actions artifact zip 内 UF2 と一致しない left/right UF2 hash、manifest 分類なのに manifest entry 証跡がない場合、manifest が指す UF2 が同じ GitHub artifact 内の UF2 entry と一致しない場合、filename 分類なのに left/right token がない場合、placeholder hash、CI 未通過、left/right の flash method 未記録、bootloader marker 未確認、書き込み直前確認未実施、keyboard half 未確認、UI smoke 未実行、token 非保存/消去 smoke 未実行、キー動作 / Combo / Trackball 編集 smoke 未実行、release wizard precondition smoke 未実行、artifact 由来表示 smoke 未実行、表示 artifact 名・id の build artifact 一致確認未実行、layer 追加・複製・参照中 layer 削除ブロック・安全な削除の smoke 未実行を拒否する。最後の公開判定では `BROWSER_FIRMWARE_PREFLIGHT_OAUTH_CLIENT_ID=<GitHub OAuth client id> npm run check:browser-firmware:public-release -- --e2e-report <report.json>` を実行し、外部 E2E 証跡 validator、merge readiness、OAuth production preflight を一括で通す。この最終 gate の preflight 対象 URL は E2E report の `production.url` と一致し、`production.url` は expected public production origin と一致し、`production.fetchUrl` は `production.url` と一致し、E2E report の `ci.runUrl` は `s-hiraoku/kobitokey-studio` の Actions run を指し、E2E report の `production.appCommitSha` は `ci.appCommitSha` と一致し、`ci.appCommitSha` は現在の git `HEAD` と一致している必要がある。独自ドメインへ移行する場合だけ `BROWSER_FIRMWARE_EXPECTED_PRODUCTION_ORIGIN` で expected origin を明示する。

...
try {
  ...
  const mismatchedCargoLock = join(root, "mismatched-cargo-lock");
  writeProject(mismatchedCargoLock, "0.1.0", { cargoLockVersion: "0.1.1" });
  const cargoLockMismatchResult = run(checker, ["--root", mismatchedCargoLock]);
  assert(cargoLockMismatchResult.status !== 0, "Expected mismatched Cargo.lock version to fail", cargoLockMismatchResult);
  assert(
    cargoLockMismatchResult.stderr.includes('src-tauri/Cargo.lock version is "0.1.1"; expected "0.1.0"'),
    "Expected mismatch output to name Cargo.lock",
    cargoLockMismatchResult,
  );
  ...
  assert(
    payload.checks.some(
      (check) => check.file === "src-tauri/Cargo.lock" && check.value === "1.2.3-beta.1" && check.pass,
    ),
    "Expected checker JSON to report updated Cargo.lock version",
    updatedCheck,
  );
...
function writeProject(projectRoot, version, options = {}) {
  ...
  writeFileSync(
    ...
    `# This file is automatically @generated by Cargo.\nversion = 4\n\n[[package]]\nname = "kobitokey-studio"\nversion = "${options.cargoLockVersion ?? version}"\ndependencies = []\n\n[[package]]\nname = "other-package"\nversion = "9.9.9"\n`,
  );
  writeFileSync(
